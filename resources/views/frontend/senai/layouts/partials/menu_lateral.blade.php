<ul class="c-14">
    <li class="sub-title sub-title-active b-20 casa-color-bg" style="text-transform: uppercase;">
        Transparência
    </li>


    <li>
        <a href="http://{{ $_SERVER['HTTP_HOST'] }}/senai/lei-de-diretrizes-orcamentarias-1" target="_self" style="text-transform: uppercase;">lei de diretrizes orçamentárias</a>
    </li>


    <li>
        <a href="http://{{ $_SERVER['HTTP_HOST'] }}/senai/demonstracoes-contabeis-1" target="_self" style="text-transform: uppercase;">demonstrações contábeis</a>
    </li>


    <li>
        <a href="#" target="_blank" style="text-transform: uppercase;">licitações e editais</a>
    </li>


    <li>
        <a href="{{ route('senai.gratuidade') }}" target="_self" style="text-transform: uppercase;">gratuidade</a>
    </li>


    <li>
        <a href="#" target="_self" style="text-transform: uppercase;">integridade</a>
    </li>


    <li>
        <a href="#" target="_self" style="text-transform: uppercase;">Dados de infraestrutura</a>
    </li>

</ul>




<ul class="c-14">
    <li class="sub-title sub-title-active b-20 casa-color-bg" style="text-transform: uppercase;">
        sac
    </li>


    <li>
        <a href="{{ route('senai.sac') }}" target="_self" style="text-transform: uppercase;">fale conosco</a>
    </li>


    <li>
        <a href="/sesi/canais/transparencia/duvidas-frequentes-faq/" target="_self" style="text-transform: uppercase;">Dúvidas frequentes (FAQ)</a>
    </li>


    <li>
        <a href="/sesi/canais/transparencia/unidades-nos-estados/" target="_self" style="text-transform: uppercase;">UNIDADES NOS ESTADOS</a>
    </li>

</ul>